...
public:
    ...
    void run(int n_steps, double dt, bool quiet=false, bool z_loop=true, int sample_step=10) {
        ...
        for (int step = 0; step < n_steps; step++) {
            ...
            for (int i = 0; i < n_ptcs; i++) {
                ...
                if (len_j > 0) {
                    ...
                    if (σhat_i_dot_g_ij > 0) {
                        ...
                        ωs.push_back(ω_ij);
                        if (accepteds[i] < ω_ij) {
                            ptcs[i].x -= σs[i][0], ptcs[i].y -= σs[i][1], ptcs[i].z -= σs[i][2];
                        }
                    }
                }
            ...
            torch::Tensor pk_mean_tensor = torch::zeros({3, 3});
            torch::Tensor qk_mean_tensor = torch::zeros({3});
            torch::Tensor dv_tensor = torch::zeros({3});
            // torch::Tensor pk_tensor = torch::zeros({3, 3});
            // torch::Tensor qk_tensor = torch::zeros({3});
            pks_tensor[step].resize(cubic_box_size);
            qks_tensor[step].resize(cubic_box_size);
            us_tensor[step].resize(cubic_box_size);
            for (int i = 0; i < cubic_box_size; i++) {
                pks_tensor[step][i].resize(cubic_box_size);
                qks_tensor[step][i].resize(cubic_box_size);
                us_tensor[step][i].resize(cubic_box_size);
                for (int j = 0; j < cubic_box_size; j++) {
                    pks_tensor[step][i][j].resize(cubic_box_size);
                    qks_tensor[step][i][j].resize(cubic_box_size);
                    us_tensor[step][i][j].resize(cubic_box_size);
                    for (int k = 0; k < cubic_box_size; k++) {
                        int len = static_cast<int>(boxes[i][j][k].ptcs.size());
                        V3 u = {0, 0, 0};
                        V3 dv = {0, 0, 0};
                        // V33 pk = {0, 0, 0, 0, 0, 0, 0, 0, 0};
                        // V3 qk = {0, 0, 0};
                        if (len > 0) {
                            for (int l = 0; l < len; l++) {
                            ...
                            u.y /= len;
                            u.z /= len;
                            pk_mean_tensor = torch::zeros({3, 3});
                            qk_mean_tensor = torch::zeros({3});
                            for (int l = 0; l < len; l++) {
                                dv.x = boxes[i][j][k].ptcs[l].vx - u.x;
                                dv.y = boxes[i][j][k].ptcs[l].vy - u.y;
                                dv.z = boxes[i][j][k].ptcs[l].vz - u.z;
                                dv_tensor = torch::tensor({dv.x, dv.y, dv.z});
                                pk_mean_tensor += torch::ger(dv_tensor, dv_tensor);
                                qk_mean_tensor += torch::dot(dv_tensor, dv_tensor) * dv_tensor;
                            }
                            pk_mean_tensor /= (len * pow(cubic_box_size, 3));
                            qk_mean_tensor /= (len * pow(cubic_box_size, 3));
                            us_tensor[step][i][j][k] = torch::tensor({u.x, u.y, u.z});
                            pks_tensor[step][i][j][k] = pk_mean_tensor;
                            qks_tensor[step][i][j][k] = qk_mean_tensor;
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < 10; ++i) {
            for (int j = 0; j < 10; ++j) {
                for (int k = 0; k < 10; ++k) {
                    file << step << "," << i << "," << j << "," << k;
                    for (int l = 0; l < 3; ++l) {
                        file << "," << cubic.us_tensor[step][i][j][k][l].item<float>();
                    }
                    for (int l = 0; l < 9; ++l) {
                        file << "," << cubic.pks_tensor[step][i][j][k][l/3][l%3].item<float>();
                    }
                    for (int l = 0; l < 3; ++l) {
                        file << "," << cubic.qks_tensor[step][i][j][k][l].item<float>();
                    }
                    file << "\n";
                }
            }
